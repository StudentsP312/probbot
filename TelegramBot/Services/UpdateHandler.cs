...
public class UpdateHandler(ITelegramBotClient bot, ILogger<UpdateHandler> logger) : IUpdateHandler
{
    ...
    )
    {
        ...
        await (
            update switch
            {
                ...
                { ChosenInlineResult: { } chosenInlineResult } => OnChosenInlineResult(
                    chosenInlineResult
                ),
    ...
    private async Task OnMessage(Message msg)
    {
        ...
        Message sentMessage = await (
            messageText.Split(' ')[0] switch
            {
                "/photo" => SendPhoto(msg),
    ...
    private async Task OnCallbackQuery(CallbackQuery callbackQuery)
    {
        logger.LogInformation(
            "Received inline keyboard callback from: {CallbackQueryId}",
            callbackQuery.Id
        );
    ...
    {
        logger.LogInformation(
            "Received inline result: {ChosenInlineResultId}",
    ...
    private async Task OnPollAnswer(PollAnswer pollAnswer)
    {
        ...
        if (pollAnswer.User != null)
            await bot.SendMessage(
                pollAnswer.User.Id,
                $"You've chosen: {selectedOption.Text} in poll"
            );
    }
